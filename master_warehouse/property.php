   ...
   if ($_POST['action_status'] == 'view_detail' | $_POST['action_status'] == 'edit_detail' | $_POST['action_status'] == 'insert_detail') {
      ...
      if (is_array($hasil) && count($hasil)) {
         foreach ($hasil as $row) :
            $v_id = $_POST['data_id'];
            $v_warehouse_code = $row['warehouse_code'];
      ...
      <table id="datatable" class="table table-striped table-bordered" style="width:100%">
         <tr>
            <td><label>Warehouse Code</label></td>
            <td><input type="text" name="code" id="jq_warehouse_code" value="' . $v_warehouse_code . '" class="form-control" ' . $is_readonly . '/> <span id="pesan_code"></span></td>
         <tr>
            <td><label>Warehouse Name</label></td>
            <td><input type="text" name="name" id="jq_warehouse_name" value="' . $v_warehouse_name . '" class="form-control" ' . $is_readonly . '/><span id="pesan_name"></span></td>
         </tr>
          <tr>
      ...
      if (is_array($hasil) && count($hasil)) {
         foreach ($hasil as $row) :
            ...
            <tr>  
               ...
               <td style="text-align: center;"><button type="button" name="view" id="' . $row["id"] . '" class="btn btn-info btn-xs view_data"><i class="fa fa-eye"></i></button> &nbsp
               <button type="button" name="edit" id="' . $row["id"] . '" class="btn btn-warning btn-xs edit_data"><i class="fa fa-pencil-square"></i></button> &nbsp
      ...
      if (is_array($hasil) && count($hasil)) {
         foreach ($hasil as $row) :
            ...
            <tr>  
               ...
               <button type="button" name="unarchive" id="' . $row["id"] . '" class="btn btn-danger btn-xs unarchive_data" ><i class="fa fa-archive"></i></button></td>
            </tr>
         ';
         endforeach;
      }
