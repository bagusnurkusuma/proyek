<?php
function get_detail_ledger_tabel($arg_start_date, $arg_end_date, $arg_account_id)
{
   $sum_debet = 0;
   $sum_credit = 0;
   $output =
...
if (!empty($_POST)) {
   ...
   if ($_POST['action_status'] == 'refresh_data_detail') {
      ...
      array(
         ...
         "account_id" => null,
         "is_show_intransit" => null
      ));
      $hasil = get_data_detail($input);
      if (is_array($hasil) && count($hasil)) {
         foreach ($hasil as $row) :
            $output .= '
            <tr id="' . $row["account_id"] . '"> 
      ...
      $output = get_detail_ledger_tabel($_POST['start_date'], $_POST['end_date'], $_POST['account_id']);
